...
import Divider from "../../components/core/divider/Divider.tsx";
...
import { useAuth } from "../../core/hooks/auth.hook.tsx";
  ...
  return (
    <DialogLayout title={titles[step - 1]}>
      <Form register={register} onSubmit={onSubmit} formState={formState} className="flex flex-col gap-2">
        ...
        <Button onClick={previousStep} color="clear" hidden={showAtStep(Steps.LOGIN)}>
          Anterior
        </Button>
...
function LoginRedirect({ hidden }: { hidden: boolean }): React.ReactNode {
  return (
    !hidden && (
      <>
        <Divider>ou</Divider>
        <Button color="clear" icon={<Button.Icon icon={FcGoogle} />}>
  ...
  return (
    <>
      ...
      <div hidden={userDataHidden}>
        ...
        <p className="text-slate-700 text-center">@username</p>
      </div>
    </>
  );
}
