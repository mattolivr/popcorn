...
export function MediaLayoutHeader(): React.ReactNode {
  return (
    <>
      <div className="flex py-4 sm:justify-start">
        <MediaLayoutPoster />
...
function MediaLayoutPoster(): React.ReactNode {
  ...
  if (media == null) {
    ...
    return <img className={className} src={mediaService.getImage(media.poster_path)} />;
  }
}
...
function MediaLayoutTagline(): React.ReactNode {
  ...
  return media?.tagline && <p className="mt-2 text-lg font-medium italic text-gray-200">{media?.tagline}</p>;
}
...
function MediaLayoutRating(): React.ReactNode {
  ...
  return (
    <div className="flex flex-col justify-start">
      ...
      <h2 className="text-left text-5xl font-extrabold text-white">{media?.getVotePercentage()}%</h2>
    </div>
  );
}
